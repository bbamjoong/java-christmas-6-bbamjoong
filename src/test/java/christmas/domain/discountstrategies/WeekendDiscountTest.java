...
class WeekendDiscountTest {
    ...
    void weekdayDiscountTest() {
        ...
        int resultDiscount = calculateDiscount(orderedFoods, 9);
    ...
    @Test
    @DisplayName("주말 디저트 메뉴 할인 X")
    void noWeekdayDiscountTest() {
        Foods orderedFoods = Foods.of(bonusGiftFoods);

        int resultDiscount = calculateDiscount(orderedFoods, 10);
    ...
    private int calculateDiscount(Foods orderedFoods, int visitDay) {
        return WeekendDiscount.of(orderedFoods, visitDay).calculate();
    }
}